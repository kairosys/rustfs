...
Kubernetes deployment module for **RustFS** (S3-compatible object storage) in a local dev cluster. This repository ships runtime data and Kubernetes manifests only — there are no Rust/Go sources, build scripts, or CI config here; it consumes the upstream container image rather than compiling from source within this repo.

## Layout

- `k8s/rustfs-deployment.yaml` — Service (`ClusterIP`, port 9000→targetPort 9000) + Deployment (replicas: 1, command `["rustfs", "/data"]`) + nginx Ingress. Image is `rustfs/rustfs:latest`; change the image/tag when you need a different version.
...
| `RUSTFS_SERVER_DOMAINS`| `s3.localhost,rustfs,rustfs.default.svc.cluster.local`        | Same; keep synced with Ingress `rules.host`    |
...
One file holds three objects: a Service, the Deployment (one replica), and an nginx Ingress. Ingress details from the source:
...
```bash
# 0. prerequisites — host path will be auto-created if absent
...
kubectl apply -f k8s/rustfs-secret.yaml       # defines name = rustfs-secret (this repo's secret object)

# 2. Service + Deployment + Ingress with env injected from this repository only:
kubectl apply -f k8s/rustfs-deployment.yaml   # Service port 9000->targetPort 9000, selector app=rustfs; Ingress routes / to s3.localhost and *.s3.localhost
```
...
```bash
kubectl get pods,svc,ingress   # Pods become Running; svc keeps 1 endpoint per pod IP matching selector app=rustfs; Ingress address includes s3.localhost / *.s3.localhost (from the rules defined above)

# Optional — RustFS exposes a health path in its HTTP layer and is not present as an object in this repo:
kubectl port-forward svc/rustfs 9000 && curl http://localhost:9000/minio/health/live   # expect HTTP 200, but it is NOT declared here

# Confirm the runtime env matches only what's declared by inline keys (no file edits required):
kubectl exec deploy/rustfs -- env | grep -E '^(RUSTFS_ADDRESS|RUSTFS_SERVER_DOMAINS)=...'   # should print: RUSTFS_ADDRESS=0.0.0.0:9000  and  RUSTFS_SERVER_DOMAINS=s3.localhost,rustfs,rustfs.default.svc.cluster.local
```

## Restore checklist (what this repo tracks vs what you bring)

- deployments target the `default` namespace;
